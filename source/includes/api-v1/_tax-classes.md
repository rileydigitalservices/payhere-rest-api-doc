...
Payhere.post("taxes/classes", data)
  .then((response) => {
...
```javascript
Payhere.delete("taxes/classes/zero-rate", {
  force: true
})
  .then((response) => {
    console.log(response.data);
...
```php
<?php print_r($payhere->delete('taxes/classes/zero-rate', ['force' => true])); ?>
```
...
```ruby
payhere.delete("taxes/classes/zero-rate", force: true).parsed_response
```
