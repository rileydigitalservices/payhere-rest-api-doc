...
Webhooks can be managed via the Payhere settings screen or by using the REST API endpoints. The `WC_Webhook` class manages all data storage and retrieval of the webhook custom post type, as well as enqueuing webhook actions and processing/delivering/logging webhooks. On `payhere_init`, active webhooks are loaded.
...
Custom topics can also be used which map to a single hook name, for example you could add a webhook with topic `action.payhere_add_to_cart` that is triggered on that event. Custom topics pass the first hook argument to the payload, so in this example the `cart_item_key` would be included in the payload.
...
Requests/responses are logged using the Payhere loggin system. Each delivery log includes:
...
Delivery logs can be accessed in "Payhere" > "Status" > "Logs".

### Visual interface ###

You can find the Webhooks interface going to "Payhere" > "Settings" > "Advanced" > "Webhooks", see our [Visual Webhooks docs](https://docs.payhere.com/document/webhooks/) for more details.
...
```json
{
  ...
  "hooks": [
    ...
    "payhere_api_edit_order",
    "payhere_order_edit_status",
    "payhere_order_status_changed"
  ],
  "delivery_url": "http://requestb.in/1g0sxmo1",
...
```php
<?php print_r($payhere->get('webhooks/142')); ?>
```
...
```ruby
payhere.get("webhooks/142").parsed_response
```
...
[
  {
    ...
    "hooks": [
      "user_register",
      "payhere_created_customer",
      "payhere_api_create_customer"
    ],
    "delivery_url": "http://requestb.in/1g0sxmo1",
    "date_created": "2016-05-15T23:17:52",
    "date_created_gmt": "2016-05-15T20:17:52",
...
print_r($payhere->put('webhooks/142', $data));
?>
```
...
payhere.put("webhooks/142", data).parsed_response
```
...
```php
<?php print_r($payhere->delete('webhooks/142')); ?>
```
...
```ruby
payhere.delete("webhooks/142").parsed_response
```
...
Payhere.post("webhooks/batch", data)
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
print_r($payhere->post('webhooks/batch', $data));
?>
```
...
payhere.post("webhooks/batch", data).parsed_response
```
  ...
  "create": [
    {
      ...
      "event": "created",
      "hooks": [
        "payhere_process_shop_coupon_meta",
        "payhere_api_create_coupon"
      ],
      "delivery_url": "http://requestb.in/1g0sxmo1",
